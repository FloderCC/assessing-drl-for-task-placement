...
public class TSPController extends Controller{
    ...
    public TSPController(String name, List<FogDevice> fogDevices , WorkflowEngine Engine) {
        ...
        Double cloud_uplink_latency = null;
        Double cloud_uplink_bandwidth = null;
        Integer cloud_id = null;

        if (cloud != null){
            cloud_id = cloud.getId();
            cloud_uplink_latency = cloud.getUplinkLatency();
            cloud_uplink_bandwidth = cloud.getUplinkBandwidth();
        }

        TSPEnvHelper.setCloudId(cloud_id);
        TSPEnvHelper.setUploadRateVariables(fog.getUplinkLatency(), fog.getUplinkBandwidth(), cloud_uplink_latency, cloud_uplink_bandwidth);
...
//        List<TSPTask> taskList = wfEngine.getJobsReceivedList();

//        for(Job job : taskList){
//            if(getcloud() != null && getDC(job.getVmId()) == getcloud().getId()){
//                count1++;
...
//        }
//
//        Job Lastjob = taskList.get(taskList.size()-1);//获取到最后一个执行的job
//        for(FileItem file : Lastjob.getFileList()){
//            if(file.getType() == FileType.OUTPUT)
//                if(getcloud() != null && getDC(Lastjob.getVmId()) == getcloud().getId())
...
//        }
        ...
        for(FogDevice fogDevice : getFogDevices())
        {
            ...
            if(name.contains("m")){
                fogDevice.setEnergyConsumption(TSPJobManager.getGatewayTotalEnergyConsumption(CloudSim.clock(), fogDevice.getHost()));//updating the energy consumption
            }
            else{
    ...
    public void print()
    {
        ...
        Log.printLine("Number of processed tasks = "+wfEngine.getJobsReceivedList().size());
...
//        System.out.println("Offloading to Cloud: "+count1+", to Fog: "+count2+".");
        ...
        TSPJobManager.printTaskExceededDeadlineQuantitiesByPriority();
        Log.printLine("===============================================================");
        Log.printLine();
    }
    ...
    public double getDatacenterCost(int id) {
        double cost = 0;
        List<TSPTask> taskList = wfEngine.getJobsReceivedList();
        for(TSPTask task : taskList){
            CondorVM vm = getVm(task.getVmId());
            PowerHost host = (PowerHost) vm.getHost();
            FogDevice fogdevice = (FogDevice) host.getDatacenter();
            int datacenterId = fogdevice.getId();
            if(!CloudSim.getEntity(datacenterId).getName().contains("m")){
                if(id == datacenterId) {
                    double c = task.getActualCPUTime()* vm.getHost().getTotalMips()/vm.getMips() * host.getcostPerMips();
                    cost += c;
                }
            }
        }
    ...
    public double getDeviceEnergy(FogDevice dev){
        ...
        for(TSPTask task:taskList){
            ...
            if(fogdevice.getId() == dev.getId()){
                executiontime += task.getActualCPUTime();
                double  e = task.getActualCPUTime() * powerModel.getPower(vm.getMips()/host.getTotalMips());
                energy += e;
            }
        }
